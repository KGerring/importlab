...
# pip._internal.commands.check.create_package_set_from_installed
# Distribution,ResourceManager,UnknownExtra
# req_file,Pep517HookCaller,
# data = self.egg_info_data('PKG-INFO');def pkg_info(self):
# egg_info_cmd = base_cmd + ['egg_info']
# name, url, extras_override = parse_editable(editable_req)
# parse_requirements,parse_editable,is_installable_dir,indent_log
# display_path,backup_dir,call_subprocess
# utils.get_similar_commands
# get_summaries,get_prog,create_main_parser,commands_dict
# cmdoptions,baseparser
# level = "DEBUG"


class Tree(dict):
    """
	return dict((p, [ReqPackage(r, index.get(r.key)) for r in p.requires()])
    ...
    def from_index(cls, index):
        c = cls(
            (p, [ReqPackage(r, index.get(r.key)) for r in p.requires()])
            for p in index.values()
        )
        c._index = index
        return c
...
class ReqPackage(Package):
    ...
    def render_as_root(self, frozen: bool = False):
        if not frozen:
    ...
    def render_as_branch(self, frozen: bool = False):
        if not frozen:
...
class NoReprDict(OrderedDict):
    @property
    def dist_index(self):
        if "dist_index" in self:
            return self.get("dist_index")
...
def construct_tree(index):
	...
	[ReqPackage(r, index.get(r.key)) for r in p.requires()]
...
def sorted_tree(tree):
    ...
    return OrderedDict(
        sorted(
            ...
            key=lambda kv: getattr(kv[0], "key", kv[0]._key),
        )
    )
...
def render_tree(tree, list_all=True, show_only=None, frozen=False, exclude=None):
    ...
    if show_only:
        nodes = [p for p in nodes if p.key in show_only or p.project_name in show_only]
    elif not list_all:
...
def get_parser():
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "This option overrides all other GLOBAL_OPTIONS."
        ),
    )
    parser.add_argument(
        "--json-tree",
        ...
        help=(
            ...
            "This option overrides all other GLOBAL_OPTIONS (except --json)."
        ),
    )
    parser.add_argument(
        "--graph-output",
...
def tree_from_list(frozen=True, indent=4, *show_only):
    if isinstance(show_only, (tuple, list, set)):
        show_only = tuple(*show_only)
    else:
        show_only = show_only
    ...
    json_ = render_json(tree, indent=indent)
    json_tree = render_json_tree(tree, indent=indent)

    rendered = render_tree(tree, list_all=False, show_only=show_only, frozen=frozen)
...
def main(*args):
    ...
    if args.warn != "silence":
        ...
        if conflicting:
            print(
                "Warning!!! Possibly conflicting dependencies found:", file=sys.stderr
            )
    ...
    if show_only and exclude and (show_only & exclude):
        print(
            "Conflicting packages found in --packages and --exclude lists.",
            file=sys.stderr,
        )
        sys.exit(1)

    tree = render_tree(
        tree if not args.reverse else reverse_tree(tree),
        list_all=args.all,
        show_only=show_only,
        frozen=args.freeze,
        exclude=exclude,
...
def DD():
    ...
    indenting = "pip._internal.basecommand.IndentingFormatter"
    ...
    D = {
        ...
        "root": {
            "level": level,
            "handlers": list(
                filter(None, ["console", "console_errors", "user_log" if log else None])
            ),
        },
